...
the quotient of TakerPays/TakerGets. It uses drops (1e-6 ICC) for ICC values.
*/

function adjustQualityForICC(quality, takerGetsCurrency, takerPaysCurrency) {
  var numeratorShift = (takerPaysCurrency === 'ICC' ? -6 : 0);
  var denominatorShift = (takerGetsCurrency === 'ICC' ? -6 : 0);
...
function parseQuality(qualityHex, takerGetsCurrency, takerPaysCurrency) {
  ...
  return adjustQualityForICC(quality, takerGetsCurrency, takerPaysCurrency);
}
